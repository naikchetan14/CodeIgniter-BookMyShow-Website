...
<body>
    ...
    <section style="height: 80vh;" class="d-flex flex-row justify-content-center align-items-center">
        ...
        <?php if (session()->getFlashdata('success')): ?>
            ...
            <?php endif; ?>
            <form class="w-25 p-4 mx-auto shadow bg-white rounded" method="POST" action="/create/account">
                <h3 class="">Create New Account For User</h3>
                <div class="mb-3">
                    <label for="name" class="form-label"> User Name</label>
                    <input type="text" class="form-control" id="name" name="name" aria-describedby="emailHelp">
                    <?php
                    if (session()->getFlashdata('errors') && isset(session()->getFlashdata('errors')['name'])): ?>
                        <div class="errors text-danger" style="font-size: 16px;"><?= esc(session()->getFlashdata('errors')['name']) ?></div>
                    <?php endif ?>
                </div>
